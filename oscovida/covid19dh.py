...
class Cache:
    def __init__(self) -> None:
        ...
        for f in os.listdir(CACHE_DIR):
            ...
            #  If a csv file in the cache is over 5 days old then remove it
            if (mtime / 86400) > 5 and os.path.splitext(cached_data_file)[1] == '.csv':
    ...
    def _cache_path(
        ...
        return os.path.join(
            CACHE_DIR, self._cache_id(level, dt, raw, vintage) + '.csv'
        )
...
class DataHub:
    def __init__(self, cache_dir: str = appdirs.user_cache_dir('covid19dh')) -> None:
        self.cache = Cache()
    ...
    @staticmethod
    def _parse_date(dt: Union[str, datetime.datetime, datetime.date]):
    ...
    def _download(
        ...
        url, filename = self._get_url(level, dt, raw, vintage)
    ...
    def get(
        ...
        if cache and self.cache.cached(level, end, raw, vintage):
            return self.cache.read(level, end, raw, vintage)

        df = self._download(level, end, raw, vintage)

        if cache:
            self.cache.write(df, level, end, raw, vintage)
